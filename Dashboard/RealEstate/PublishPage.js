...
const PropertyDetailPage = () => {
    ...
    const fetchPropertyDetails = async () => {
      try {
        ...
        if (user_id && property_id) {
          ...
          console.log('Image field:', data?.property?.image);
        } else {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      {propertyData?.image && propertyData.image.includes('.jpg') ? ( 
        <FlatList data={propertyData.image.split(',')} 
        horizontal keyExtractor={(item, index) => index.toString()} 
        renderItem={({ item }) => ( <Image source={{ uri: item.trim() }} 
        style={styles.image} /> )} showsHorizontalScrollIndicator={false} 
        style={styles.imageScroll} />
         ) : ( <Image source={{ uri: 'https://upload.wikimedia.org/wikipedia/commons/d/d1/Image_not_available.png' }} style={styles.image} /> )} 
      ...
      <View style={styles.featuresRow}>
        ...
        {propertyData?.carparking && (
          <Feature icon="car" label={`${propertyData.carparking} Car Parking`} />
        )}
      </View>
...
<View style={styles.details}>
  ...
  onPress={() => navigation.navigate('UploadDetailsScreen', { editMode: true })}
>
...
const Feature = ({ icon, label }) => (
 ...
 const styles = StyleSheet.create({ 
  ...
  image: { width: '100%', height: 180, borderRadius: 12, marginVertical: 10 },
  saleRow: { flexDirection: 'row', justifyContent: 'space-between', 
  alignItems: 'center' }, saleTag: { flexDirection: 'row', alignItems: 'center' }, 
  greenDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: 'green', marginRight: 6 },
  price: { fontSize: 18, fontWeight: 'bold' },
  featuresRow: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 16 },
  featuresRow1: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 16 },
  featureItem: { alignItems: 'center', flex: 1 }, 
  sectionTitle: { fontWeight: 'bold', marginTop: 12, marginBottom: 4,fontSize:18,color: 'black' }, 
  description: { color: 'black', fontSize: 14, lineHeight: 20 }, 
  locationRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 4 }, 
  locationHighlightTitle: { fontWeight: 'bold', marginTop: 8 }, 
  buttonRow: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 60 },
  editButton: { backgroundColor: '#fff', borderColor: '#f60138', borderWidth: 1, paddingVertical: 10, paddingHorizontal: 70, borderRadius: 30, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation:6, }, 
  publishButton: { backgroundColor: '#f60138', paddingVertical: 10, paddingHorizontal: 70, borderRadius: 30, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.3, shadowRadius: 4, elevation:6, }, buttonText: { color: '#fff', fontWeight: 'bold' }, 
  editbuttonText: { color: '#88888', fontWeight: 'bold' }, 
  imageScroll: { marginVertical: 10, }, 
  image: { width: 380, height: 190, borderRadius: 12, marginRight: 10, }, 
  priceTag:{ color: '#555', fontSize: 14, lineHeight: 20 }, 
  detailsRow: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 6, borderBottomWidth: 0.5, borderBottomColor: '#ccc', marginHorizontal: 10,marginTop:10, }, 
  detailLabel: { fontWeight: 'bold', color: '#88888', width: '50%',fontSize:15, }, 
  detailValue: { color: '#333', width: '50%', textAlign: 'right', }, 
  spacer:{height:50,},
  details:{
    marginTop:20,
  },
